...
KytaDrillCharge = WeaponAbility:new()

function KytaDrillCharge:init()
  self:reset()
end

function KytaDrillCharge:update(dt, fireMode, shiftHeld)
...
function KytaDrillCharge:windup()
...
function KytaDrillCharge:charge()
...
function KytaDrillCharge:winddown()
...
function KytaDrillCharge:reset()
  status.clearPersistentEffects("weaponMovementAbility")
  animator.setAnimationState("drill", "idle")
  animator.setAnimationState("energy", "idle")
end

function KytaDrillCharge:uninit()
  self:reset()
end

function KytaDrillCharge:damageTiles()
  local pos = mcontroller.position()
  local tipPosition = vec2.add(pos, activeItem.handPosition(animator.partPoint("spear", "drillTip")))
  for i = 1, 3 do
    local sourcePosition = vec2.add(pos, activeItem.handPosition(animator.partPoint("spear", "drillSource" .. i)))
